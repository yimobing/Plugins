	...
	*/
	function createMapPoint(dataJson,paramJson){
		...
		var l_point = dataJson.l_point,
			...
			cityNo = dataJson.cityNo,
			locate = dataJson.locate, 
		...
		//自定义marker标注点属性，供外部调用 add 20181229-1
		marker.customJson = { "title":title, "dataType": dataType, "dataNo":dataNo, "cityNo":cityNo, "locate":locate};

		if($openDotGather){ //edit 20181229-2(是否开启点聚合)
			if(typeof(markers)!='undefined') markers.push(marker); //添加点聚合组成的数组
		}
		...
		if($openDotGather){
			marker.disableMassClear(); //禁止覆盖物在 map.clearOverlays 方法中被清除 
			//marker.enableMassClear(); //允许清除
		}else{
			//marker.disableMassClear(); //禁止覆盖物在 map.clearOverlays 方法中被清除 
			marker.enableMassClear(); //允许清除
		}
	...
	*/
	function createMapLabel(marker,json){
		...
		if(typeof(title)!='undefined'){
			...
			var tStyle = title=='' ? ' no-padding' : ''; //add 20181023-1
			var label = new BMap.Label('<div class="bdLabel '+style+tStyle+'">'+title+'</div>', l_opts);  // 创建文本标注对象 edit 20181023-1
	...
	*/
	function createMapInfoWindow(marker,json){
		var title = json.title,
			description = typeof(json.description)=='undefined' ? title : json.description,
			point = json.point;
		...
		if(typeof(description)!='undefined'){
			marker.addEventListener("click", function(e){  //mouseover鼠标经过时,click鼠标点击时
				...
				//获取自定义的属性并将值传递给前台 add 20181229-1
				var webJson = e.target.customJson;
				//console.log('里面自定义属性:',webJson);
				clickPointFunc(webJson); 
