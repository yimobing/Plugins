...
;(function(root, factory) {
    ...
    jedfn.initdates = function(){
        ...
        var formatDate = function (cls,boxcell) {
            var dateDiv = $("<div/>",{"id":boxcell.replace(/\#/g,""),"class":"jedatebox "+(opts.skinCell || config.skinCell)+' '+(opts.theme || config.theme)}),
            ...
            if(opts.range == false){
                var hmsVal = that.gethmsVal(boxCell),
                    dateVal = function () {
                        ...
                        //var spval = jet.reMatch(boxCell.find(ymday).find("td.actdate").attr("je-val"));
                        //edit by chr 20191120-1 修正“勾选年月时，不选具体某天，而直接点【确定】会出现错误”的bug。
                        if(typeof boxCell.find(ymday).find("td.actdate").attr("je-val") != 'undefined'){
                            var spval = jet.reMatch(boxCell.find(ymday).find("td.actdate").attr("je-val"));
                        }
                        else{
                            var spval = jet.reMatch(boxCell.find(".daybox").find(".arthead").attr("je-ym") + '-1'); //默认勾选年月的那个月的第1天
                        }
    ...
    jedfn.dateOrien = function(boxCls, valCls, pos) {
        ...
        //boxCls.css({"top":ortop,"left":orleri}); //edit 日历位置 top left by chr 20180806-1
				
				/** （自适应PC、移动端)判断当前是移动端还是PC端,若移动端flag=true add by chr 20180806-1 以下11行*/
				...
				if(!flag) boxCls.css({"top":ortop,"left":orleri}); //PC端
				else $('.jedatebox').addClass('mobile');
